 ...
 *  Version 1.10c
 *      - Changed the SetNumberWidth(0) to SetNumberWidth(5) in Initialize();
 *  Version 1.10b
 *      - Replaced "Sig" with "*Sig" for nets with more than 8 elements
 *        Also added "*End" to the end of the connection list
 *      - Removed "*Standard" from the connection list
 *      - Corrected capacity note regarding Node Name length
 *      - Added Version message
 *      - Corrected problem regarding net names that are too long with
 *        more than 8 elements in a net
 *      - Recorrected problem regarding net names that are too long with
 *        more than 8 elements in a net!!!
 *
 *  Capacities:
 ...
 *      - References can only use these characters { 'A-Z', 'a-z', '0-9' }
 */
...
WriteNet()
{
    ...
    if (net_items == 8)
    {
        ...
        /* if the name contains illegal characters or is
         * too long, use net number
         */
        legal = SymbolInCharSet(SignalNameStr);
        if (legal != 1)
            { i = 'N'; }
